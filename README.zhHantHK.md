...
- **媒體上傳/下載** — 檔案、圖片、影片，自動類型偵測
- **訊息管理** — 撤回、動態卡片更新
- **群組 V2** — 建立、資訊、成員、列表、成員檢查、更新設定與成員
- **行事曆與排程** — 主行事曆、排程 CRUD、出席人管理
...
result, err := client.SendMarkdown(ctx, "staff123", "**粗體**", false, nil, false, "", "")
...
// 可用事件類型
types := lansenger.GetCallbackEventTypes() // 14 個類別下的 24 種事件類型
```

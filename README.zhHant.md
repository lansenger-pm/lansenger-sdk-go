...
| API Gateway URL | ✓ | `api_gateway_url` | API 閘道地址（預設：`https://open.e.lanxin.cn/open/apigw`） |
...
| `LANSENGER_API_GATEWAY_URL` | ✗ | API 閘道地址 | `https://open.e.lanxin.cn/open/apigw` |
...
| `message send-text --group` (群聊) | **Y** | N | **Y** | 個人機器人 API 支援，但暫不支援入群 |
